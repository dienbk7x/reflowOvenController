...
//constructors
TFT_ILI9163C::TFT_ILI9163C(const uint8_t& cspin,const uint8_t& dcpin,const uint8_t& rstpin) : Adafruit_GFX(_TFTWIDTH,_TFTHEIGHT){
    _cs   = cspin;
	_rs   = dcpin;
	_rst  = rstpin;
...
TFT_ILI9163C::TFT_ILI9163C(const uint8_t& CS, const uint8_t& DC) : Adafruit_GFX(_TFTWIDTH, _TFTHEIGHT) {
    _cs   = CS;
	_rs   = DC;
	_rst  = 0;
	#if defined(__MK20DX128__) || defined(__MK20DX256__)
...
void TFT_ILI9163C::begin(SPIClass* spi) {
    _SPI = spi;
    begin();
}
...
#elif defined(__MK20DX128__) || defined(__MK20DX256__)
	_SPI->begin();
	if (_SPI->pinIsChipSelect(_cs, _rs)) {
		pcs_data = _SPI->setCS(_cs);
		pcs_command = pcs_data | _SPI->setCS(_rs);
	} else {
		pcs_data = 0;
		pcs_command = 0;
		return;
...
#else
    _SPI->dmaSend(_scanlineBuffer8, n << 1);	// each pixel is 2 bytes
#endif
}
