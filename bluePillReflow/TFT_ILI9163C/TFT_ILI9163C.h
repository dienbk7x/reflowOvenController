...
class TFT_ILI9163C : public Adafruit_GFX {
 ...
 public:
	...
	TFT_ILI9163C(const uint8_t& cspin, const uint8_t& dcpin, const uint8_t& rstpin);
 ...
 private:
	uint8_t		_Mactrl_Data = 0;//container for the memory access control data
	uint8_t		_colorspaceData = 0;
	...
	void				spiwrite(uint8_t);
	volatile uint32_t 	*dataport = 0, *clkport = 0, *csport = 0, *rsport = 0;
	uint8_t 			_cs = 0,_rs = 0,_sid = 0,_sclk = 0,_rst = 0;
	uint32_t  			datapinmask = 0, clkpinmask = 0, cspinmask = 0, rspinmask = 0;
