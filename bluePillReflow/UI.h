...
void displaySplash() {
	...
	tft.setCursor(5, 80);
	tft.println("(c)2014 0xPIT");
	tft.setCursor(5, 90);
	tft.println("(c)2017 Dasaki");
	tft.setCursor(5, 100);
	tft.println("(c)2018 Victor Perez");
...
extern const Menu::Item_t miProfileName,
miRampToSoakTime, miRampToSoakTemp, miSoakTime, miSoakTemp,
miRampUpTime, miRampUpTemp, miPeakTime, miPeakTemp,
miRampDownTime, miRampDownTemp, miCoolDownTime, miCoolDownTemp,
miLoadProfile, miSaveProfile, miloadRomProfile,
...
/*
 * This functions edit the name of the profile.
 * Profiles can be saved to eeprom of flash depending on settings
 */
bool menu_editName(const Menu::Action_t action) {
	if (action == Menu::actionDisplay) {
		...
		if (initial) {
			tft.fillScreen(WHITE);
			buf_idx = 0;
			strncpy(buf, activeProfile.name, sizeof(activeProfile.name));
			tft.setTextColor(BLACK, WHITE);
			tft.setCursor(MENU_TEXT_XPOS, 40);
			tft.print("Edit & click to next");
			tft.setCursor(MENU_TEXT_XPOS, 49);
			tft.print("Doubleclick to exit");
			tft.setCursor(MENU_TEXT_XPOS, 58);
			tft.print("Save the profile to");
			tft.setCursor(MENU_TEXT_XPOS, 67);
			tft.print("make it permanent");
		...
		encAbsolute = constrain(encAbsolute, ' ', 'z');
		//tft.setCursor(10, 80);
		if (encAbsolute != encLastAbsolute) {
			encLastAbsolute = encAbsolute;
			buf[buf_idx] = (char)encAbsolute;
			tft.setCursor(10, 29);
			tft.setTextColor(WHITE, BLUE);
			for (uint8_t n = 0; n < sizeof(activeProfile.name); n++){
				if (n != buf_idx) tft.print(buf[n]);
				else {
	 ...
	 */
	else if (action == Menu::actionTrigger) { // increase the buf_idx
		buf_idx = (buf_idx + 1) % (NAME_LENGTH-1);
		encAbsolute = buf[buf_idx];
		encLastAbsolute = encAbsolute-1;
		return true;
	}
		...
		if (currentState == Edit) {
			strncpy (activeProfile.name, buf, sizeof(activeProfile.name));
			currentState = Settings;
			clearLastMenuItemRenderState();
			tft.fillRect(10, 18, tft.width(), 64, WHITE);
			return false;
		}
	}
	return true;
...
bool menu_editNumericalValue(const Menu::Action_t action) { 
	...
	if (action == Menu::actionParent || action == Menu::actionTrigger) {
		...
		if (currentState == Edit) { // leave edit mode, return to menu
			...
			tft.fillRect(0, 85, tft.width(), 10, WHITE);
			currentState = Settings;
...
void loadRomProfile(uint8_t& i){
	i = min(i, (sizeof( romProfiles ) / sizeof( romProfiles[0])));
...
bool menu_factoryReset(const Menu::Action_t action) {
	if (action == Menu::actionDisplay) {
		...
		if (initial) { // TODO: add eyecandy: colors or icons
			...
			tft.setTextSize(1);
			tft.setCursor(0, 10);
			tft.println("This will reset PID");
			tft.print("and first ");
			tft.print(sizeof( romProfiles ) / sizeof( romProfiles[0]));
			tft.println(" profiles");
			tft.println("settings to default");
			tft.println();
		...
		if (currentState == Edit) { // leave edit mode only, returning to menu
			tft.fillRect(0, 18, tft.width(), 64, WHITE);
			//  tft.fillScreen(WHITE);
			currentState = Settings;
			clearLastMenuItemRenderState();
			return false;
...
bool menu_loadRomProfile(const Menu::Action_t action) {
	uint8_t i;
	if (action == Menu::actionDisplay) {
		bool initial = currentState != Edit;
		currentState = Edit;
		...
		encAbsolute = constrain(encAbsolute, 0, (int16_t)(sizeof( romProfiles ) / sizeof( romProfiles[0]))-1);
		...
		if (encAbsolute != encLastAbsolute) {
			...
			//change this loadProfileName(encAbsolute, buf);
			strcpy (buf,  romProfiles[i].name);
			tft.fillRect(9, 27, (NAME_LENGTH * 6 + 1), menuItemHeight, BLUE);
			tft.setCursor(10, 29);
			tft.setTextColor(WHITE, BLUE);
			tft.print(buf);
...
MenuItem(miAutoTune,  "Autotune",  miProfileName, miCycleStart, miExit, miTuneStart, menuDummy);
...
MenuItem(miProfileName, "Edit Prof. Name", miEditProfile, miAutoTune, miExit, Menu::NullItem, menu_editName);
MenuItem(miEditProfile, "Edit Profile", miLoadRomProfile, miProfileName,   miExit, miRampToSoakTime, menuDummy);
...
MenuItem(miLoadRomProfile,  "Load Rom Profile",  miLoadProfile,  miEditProfile, miExit, Menu::NullItem, menu_loadRomProfile);
MenuItem(miLoadProfile,  "Load Profile",  miSaveProfile,  miLoadRomProfile, miExit, Menu::NullItem, menu_saveLoadProfile);
