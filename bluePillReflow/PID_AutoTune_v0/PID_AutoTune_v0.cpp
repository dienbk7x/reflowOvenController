...
float inline PID_ATune::fastArcTan(float x)
{
...
float PID_ATune::calculatePhaseLag(float inducedAmplitude)
{ 
  // calculate phase lag
    ...
    if (stepCount > 4)
    {
      ...
      if ((avgStep1 > 1e-10) && (avgStep2 > 1e-10))
      {
        ...
        if (asymmetry > AUTOTUNE_STEP_ASYMMETRY_TOLERANCE)
        {
          ...
          float deltaRelayBias = - processValueOffset(avgStep1, avgStep2) * workingOstep;
...
#if defined (AUTOTUNE_RELAY_BIAS)
float PID_ATune::processValueOffset(float avgStep1, float avgStep2)
{
